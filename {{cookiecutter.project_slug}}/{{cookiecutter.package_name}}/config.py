...
from {{ cookiecutter.package_name }}.log import Log


class Config:
    def __init__(self):
        self.path = Path("~/.config/{{ cookiecutter.package_name }}").expanduser()
        self.path.mkdir(parents=True, exist_ok=True)
        self.dotenv_path = self.path / "config"
        self.logger = Log()
        self.keys_dict = {
            "first_var": {"name": "{{ cookiecutter.package_name_upper }}_VAR", "note": "first variable"},
